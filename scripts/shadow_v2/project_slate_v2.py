...
from nfl_edge.projection.store import DIRNAME, ProjectionConflict, ProjectionStore         # noqa: E402
...
def main(argv=None):
    ...
    store = ProjectionStore(os.path.join(a.out, DIRNAME))
    written = {}
...
def build_player_arms(a, quotes, qs, sched, gidx, run_ts, now) -> PlayerArms:
    ...
    for i, r in feat.iterrows():
        P.feat[(r["player_id"], r["game_id"])] = {c: (r[c] if c in feat.columns else None) for c in
                                                  ("team", "ewma_team_pass_att", "ewma_team_rush_att", "ewma_target_share", "ewma_carry_share",
                                                   "ewma_snap_share", "qb_changed_recent", "n_prior", "shrink_w")}
