...
1994 player-stat-game projections diagnosed. Ranked by |robust z| of the actual inside the fitted distribution (cross-statistic), ties by game and player. Classification is deterministic from the recorded intermediates.
...
Classification counts over every diagnosed projection: {'EFFICIENCY_MISS': 349, 'UNEXPLAINED_VARIANCE': 62, 'TEAM_VOLUME_MISS': 74, 'OPPORTUNITY_MISS': 687, 'NO_LARGE_MISS': 765, 'INSUFFICIENT_DATA': 37, 'AVAILABILITY_MISS': 20}.
Missing usage (no snap table or stats row): 57.

## Decision funnel (BET / WATCH / PASS accounting of the existing gates; no authority)

61 snapshot(s). A contract's terminal state is where it left the existing gate sequence when the model's own contract value stands in for a handicap. BET here means every mechanical gate the preflight applies would pass; it is still not a recommendation (that needs a human handicap and the signed preflight).

### Newest snapshot 20260923T075204Z (39816 markets)
...
| supported | 2251 |
| priced | 2251 |
| raw_disagreement | 1580 |
| tradable_book | 1531 |
| data_quality | 1529 |
| executable_price | 937 |
| liquidity | 246 |

Terminal states: {'PASS': 38979, 'WATCH': 591, 'BET': 246}

| family | BET | WATCH | PASS |
|---|---|---|---|
| AWARD | 0 | 0 | 868 |
| BOTH_TEAMS_SCORE | 0 | 0 | 132 |
| BOTH_TEAMS_SCORE_N | 0 | 0 | 132 |
| COACH_EVENT | 0 | 0 | 64 |
| CONFERENCE_WINNER | 0 | 0 | 32 |
| DIVISION_WINNER | 0 | 0 | 32 |
| DRAFT | 0 | 0 | 40 |
| FIRST_TD_SCORER | 0 | 0 | 991 |
| FIRST_TD_TEAM | 0 | 0 | 970 |
| GAME_EVENT | 0 | 0 | 149 |
| GAME_PLAYER_LEADER | 0 | 0 | 329 |
| GAME_STAT | 0 | 0 | 5 |
| GAME_WINNER | 0 | 37 | 91 |
| HALF_FULL_RESULT | 0 | 0 | 423 |
| MAKE_PLAYOFFS | 0 | 0 | 33 |
| NFL_BUSINESS_EVENT | 0 | 0 | 5 |
| NOT_NFL_OR_UNKNOWN | 0 | 0 | 2 |
| PERIOD_WINNER | 0 | 0 | 846 |
| PLAYER_AVAILABILITY | 0 | 0 | 23 |
| PLAYER_ROLE_EVENT | 0 | 0 | 177 |
| PLAYER_STAT | 234 | 103 | 13385 |
| RACE_TO_N | 0 | 0 | 540 |
...
| SEASON_WINS | 0 | 0 | 547 |
| SPREAD | 8 | 181 | 4799 |
| SUPER_BOWL_EVENT | 0 | 0 | 235 |
| SUPER_BOWL_WINNER | 0 | 0 | 32 |
| TEAM_STAT | 0 | 0 | 946 |
| TEAM_TOTAL | 4 | 125 | 2221 |
...
| WEEK_EVENT | 0 | 0 | 32 |
| WEEK_LEADER | 0 | 0 | 359 |
| WIN_MARGIN_BUCKET | 0 | 0 | 231 |

| rejection reason | n |
|---|---|
| POST_KICKOFF_EXCLUDED | 23399 |
| unmapped | 8280 |
| UNSUPPORTED_MODEL | 3833 |
| UNSUPPORTED_RULES | 2045 |
| no order book observed for this ticker (books are captured i | 691 |
| no positive disagreement against either executable ask | 671 |
| book not tradable for ranking | 49 |
| ask 0.98 above the zero-net-EV ceiling 0.97 | 21 |
| ask 0.99 above the zero-net-EV ceiling 0.98 | 17 |
| ask 0.83 above the zero-net-EV ceiling 0.81 | 10 |
| ask 0.95 above the zero-net-EV ceiling 0.94 | 10 |
| ask 0.09 above the zero-net-EV ceiling 0.08 | 9 |
| ask 0.06 above the zero-net-EV ceiling 0.05 | 9 |
| ask 0.04 above the zero-net-EV ceiling 0.03 | 9 |
...
Across all 61 snapshots (repeated markets counted per snapshot): {'PASS': 1681691, 'WATCH': 41326, 'BET': 34252}.
