...
| statistic | w (weeks 1–9) | confirmation weeks 10–22: reconciled − market Brier (z) | w (all 2025) | **deployed** | why |
|---|---|---|---|---|---|
| any_td | 0.60 | −0.00007 (z −0.12) | 0.70 | **0.60** | confirmed; smaller optimum |
| rush_yards | 0.25 | +0.0015 (z +0.84) | 0.05 | **0.05** | confirmed within z ≤ 1; smaller optimum |
| rec_yards | 0.00 | +0.0014 (z +1.61) at w = 0 | 0.05 | **0** | early optimum was 0 |
| receptions | — (too few identified ladders in weeks 1–9) | — | 0.15 | **0** | no early fit to confirm |
| pass_yards | 0.00 | +0.0028 (z +1.99) at w = 0 | 0.00 | **0** | early optimum was 0 |
| pass_td | — (too few rows) | — | 1.00 | **0** | no early fit to confirm; **the top candidate for a preregistered follow-up** |

A deployed weight of 0 means the reconciled distribution sits at the market mean with the football shape;
it is reported and never ranked. Note that even at w = 0 the reconciled probability is slightly worse than
the monotone midpoint on receiving yards and passing yards (+0.0014, +0.0028): the football *shape* at the
market mean is not better than the market's own ladder, which is itself a finding.
...
| any_td | 0.05–0.10 | 372 | 0.4 | 0.1907 | 0.1943 |
...
reconciled disagreement: at w = 0.6 for anytime TD the largest reconciled deviations are ~0.05, not 0.4.
...
and the weights fitted at T-24h are the same order as at T-0 (any_td 0.45, rush 0.15, pass_yards 0.15,
rec 0.0). The deployed weights are the T-0 ones, the conservative choice.
