...
| **R** coherent game-simulation projection layer | **NEW, built 2026-09-16, shadow** — `nfl_edge/sim`: one Monte Carlo game per slate game (score → script-conditional volume → Dirichlet-multinomial opportunity over the point-in-time eligible set → opponent-adjusted per-touch efficiency → touchdown allocation), every player output reconciled to its team, one distribution per Kalshi ladder, market reconciliation weights fitted out of sample on the 2025 archive. Walk-forward 2023–2025 and the Week-1 diagnostic in `research/simulation_engine/`; `docs/SIMULATION_ENGINE.md` | accrue prospective 2026 rows from Week 2 in `data/shadow/sim`; promotion is a separate, explicit decision |
