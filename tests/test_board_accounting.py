...
# A series the classifier genuinely does not know, and which the registry therefore cannot hold.
UNKNOWN_SERIES = "KXNFLBRANDNEWTHING"
...
def _disc():
    ...
    # A brand-new unknown series, exactly the shape of the drift the audit found. It has to be a series the
    # taxonomy genuinely does not know: KXNFLPOTM used to stand here and is now classified as AWARD from its
    # own rules text (one of nine the 2026-09-19 drift report caught the registry lagging on), so it no
    # longer exercises the unclassified path at all.
    by_series[UNKNOWN_SERIES] = [{"ticker": f"{UNKNOWN_SERIES}-SEP26NFCOFFENSE-TMCMILLAN4", "event_ticker": f"{UNKNOWN_SERIES}-SEP26NFCOFFENSE",
                                  "series_ticker": UNKNOWN_SERIES, "title": "A brand-new market Kalshi has just listed",
                                  "strike_type": "structured", "custom_strike": {"football_player": "u1"}, "status": "active"}]
    series_nfl = [{"ticker": s, "title": (REG.get(s) or {}).get("title") or ("A Brand-New Pro Football Market" if s == UNKNOWN_SERIES else s), "tags": ["Football"],
                   "product_metadata": {"scope": "Monthly Awards"} if s == UNKNOWN_SERIES else None} for s in by_series]
...
def test_every_contract_terminates_and_unknown_series_are_not_captured_as_is():
    ...
    potm = rows[f"{UNKNOWN_SERIES}-SEP26NFCOFFENSE-TMCMILLAN4"]
    assert potm["terminal_state"] == S.NOT_CAPTURED
    assert "absent from the registry" in potm["terminal_reason"]
...
def test_provisional_capture_turns_not_captured_into_capture_only():
    disc = _disc()
    series = next(s for s in disc["series_nfl"] if s["ticker"] == UNKNOWN_SERIES)
    rec = P.provisional_record(series, open_markets=1, now=NOW)
    assert rec["tier"] == "LIGHT" and rec["semantic_confidence"] == "UNKNOWN"
    ...
    potm = next(r for r in board["rows"] if r["ticker"].startswith(UNKNOWN_SERIES))
    ...
    merged2 = P.merge_provisional(merged, [], {**REG, UNKNOWN_SERIES: {"tier": "DAILY"}}, now=NOW)
    assert merged2["series"][UNKNOWN_SERIES]["status"] == "PROMOTED_TO_REGISTRY" and merged2["series"][UNKNOWN_SERIES]["first_seen"] == rec["first_seen"]


def test_drift_report_names_new_series_lag_and_capture_gap():
    disc = _disc()
    rep = D.drift_report(disc, REG, capture_state={"last_seen": {}, "fingerprints": {}}, first_seen={UNKNOWN_SERIES: "20260901T090000Z"}, now=NOW)
    new = {r["series"]: r for r in rep["NEW_SERIES"]}
    assert UNKNOWN_SERIES in new and new[UNKNOWN_SERIES]["registry_lag_days"] > 4
    assert any(r["series"] == UNKNOWN_SERIES for r in rep["UNCLASSIFIED"])
    assert any(r["series"] == UNKNOWN_SERIES and not r["in_registry"] for r in rep["CAPTURE_GAP"])
    assert rep["totals"]["new_series"] >= 1
    md = D.render_drift(rep)
    assert UNKNOWN_SERIES in md
