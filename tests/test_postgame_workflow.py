...
def test_J4_no_recorded_research_result_is_rewritten():
    ...
    # The registry INDEX is a generated projection of the per-hypothesis JSON files (research/hypothesis_registry/
    # README.md: "regenerated by rebuild_registry.py ... do not edit by hand"); registering a new hypothesis
    # necessarily rewrites it. The JSON files themselves stay append-only and are still checked here.
    derived = {"research/hypothesis_registry/registry.md"}
    offenders = [p for status, p in changed if status != "A" and p not in derived]
